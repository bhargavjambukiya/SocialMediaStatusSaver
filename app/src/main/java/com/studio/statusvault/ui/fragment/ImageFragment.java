...
public class ImageFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if ("recent".equals(FilesData.getRecentOrSaved())) {
            if (recentAdapter == null) {
                recentAdapter = new RecentStatusMediaListAdapter(requireContext(), 'i', null);
                recyclerView.setAdapter(recentAdapter);
            }
            viewModel.getRecentImagesStatusFolderOnly()
                    .observe(getViewLifecycleOwner(), this::bindRecentImageList);
        } else {
            if (savedAdapter == null) {
                savedAdapter = new SavedFileListAdapter(requireContext(), 'i');
                recyclerView.setAdapter(savedAdapter);
            }
            viewModel.getSavedImages().observe(getViewLifecycleOwner(), this::bindSavedImageList);
        }

        return v;
    }

    private void bindRecentImageList(ArrayList<StatusMediaItem> files) {
        if (files == null || files.isEmpty()) {
            ...
            int emptyRes = RecentStatusRangePreference.isLast7DaysOnly(requireContext())
                    ? R.string.status_empty_recent_images_last_7_days
                    : R.string.status_no_recent_images;
    ...
    private void bindSavedImageList(ArrayList<File> files) {
        if (files == null || files.isEmpty()) {
            layoutNoRecordFound.setVisibility(View.VISIBLE);
            if (savedAdapter != null) {
