...
public class AudioExtractor {
    ...
    public void genVideoUsingMuxer(File srcPath, int startMs, boolean useAudio, boolean useVideo, Dialog mProgressDialog) throws IOException {
        ...
        // Set up MediaMuxer for the destination (app-specific external dir — no legacy storage write permission).
        MediaMuxer muxer;
        File parentDir = FilesData.getAppAudioDir();
        String destinationPath = new File(parentDir, srcPath.getName()).getAbsolutePath();
        ...
        muxer.stop();
        muxer.release();
        File from = new File(parentDir, srcPath.getName());
        String baseName = srcPath.getName();
        String mp3Name;
        int lastDot = baseName.lastIndexOf('.');
        if (lastDot > 0) {
            mp3Name = baseName.substring(0, lastDot) + ".mp3";
        } else {
            mp3Name = baseName + ".mp3";
        }
        File to = new File(parentDir, mp3Name);
        if (!from.renameTo(to)) {
            Log.w(TAG, "Could not rename muxer output to " + to.getAbsolutePath());
        }
